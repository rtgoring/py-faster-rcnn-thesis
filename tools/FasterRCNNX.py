...
INPUT_MODE = 2 #0 = images, 1 = Camera, 2 =Video, 3= ScreenCapture
GUI = 1
RECORD = 0
PRINT_STATS = 0
...
def demo_video(net, im):
    ...
    cv2.putText(im,'Num Objects: {:d}'.format(len(found_objects)),(50,50),cv2.FONT_HERSHEY_SIMPLEX,.75,(255,0,255))#,cv2.CV_AA
    count =3
    for fObject in found_objects:
        #Prints info about all objects
        cv2.putText(im,'{:s} - L: {:d} - D: {:d}'.format(fObject.name, fObject.life, fObject.time_not_seen),(50,25*count),cv2.FONT_HERSHEY_SIMPLEX,.75,(255,0,255))#,cv2.CV_AA
        count +=1
        if fObject.inFrame: #Prints info for only currently seen objects
            cv2.putText(im,'{:s} - Life: {:.0f} - Loc: ({:.0f},{:.0f},{:.0f})'.format(fObject.name, fObject.life,fObject.rl_location[0],fObject.rl_location[1],fObject.rl_location[2]),(int(fObject.px_location[0][0]),int(fObject.px_location[0][1])-50),cv2.FONT_HERSHEY_SIMPLEX,.75,(255,0,255))#,cv2.CV_AA)
            cv2.putText(im,'Width: {:1f} - Avg Width: {:.1f}'.format(fObject.width, fObject.average_size),(int(fObject.px_location[0][0]),int(fObject.px_location[0][1])-75),cv2.FONT_HERSHEY_SIMPLEX,.75,(255,0,255))#,cv2.CV_AA
            cv2.putText(im,'Distance: {:1f} - Area: {:.1f}'.format(fObject.distance, fObject.area),(int(fObject.px_location[0][0]),int(fObject.px_location[0][1])-100),cv2.FONT_HERSHEY_SIMPLEX,.75,(255,0,255))#,cv2.CV_AA
            ...
            cv2.rectangle(im,(fObject.x1_average,fObject.y1_average),(fObject.x2_average,fObject.y2_average),(255,100,255),2)
...
if __name__ == '__main__':
	...
	videoFileExt = '.avi'
	videoFile = cv2.VideoCapture(videoFilePath+videoFileName+videoFileExt)
	while True:
